...
int main(int argc, char **argv) 
{
   ...
   if ((argc != 4)) {
	 ...
	 omp_set_num_threads(atoi(argv[1])); //por algum motivo o get retorna 1, por isso manti a variável nthreads
   nthreads = atoi(argv[1]);           //fiz testes e pelo que vi as threads executam de forma correta
   wsize = atoi(argv[2]);
   repeat = atoi(argv[3]);
